...
with open('roco_filtered.json', 'r') as f:
    data = json.load(f)

# data = data[:40]
print("Number of samples being processed: ", len(data))
limit_count = 40
print("Limiting the number of samples to: ", limit_count)
...
for item in tqdm(data):
    try:
      ...
      print("Generating a report from the image and rough description")
      ...
      system_prompt_content_for_report_generation = "You are an AI assistant with advanced capabilities in radiology. \
                              Your task is to analyze the attached medical image and the associated rough description of it. \
      ...
      user_prompt_content_report_generation = "Please process this image attached. Here is a rough description of it saying - |||" + item + "|||. I need you to analyze an image, and prepare a detailed report out of it using both your findings and information from the rough description. \
      ...
      system_prompt_content_for_VQA_format = "You are an AI assistant with advanced capabilities in formatting inputs for VQA datasets. \
                              Your task is to convert the given plain text input into a VQA dataset format. \
                              You need to create a dictionary containing two keys: 'prompt' and 'response'. \
                              The 'response' key should contain the given plain text input as it is. \
                              The 'prompt' key should be a well-crafted prompt that says what the image is of and asks the model to generate a detailed report of the image. \
                              Example Response: '\
                              {'prompt': 'Here is a CT Scan of the liver. Describe the given image in detail in a report format or something like what are the key findings in this attached xray/ct/MRI scan? Prepare a detailed report of the image.', \.', \
                              'response': Given plain text input}'"
      
      # User prompt for VQA dataset format conversion
      user_prompt_content_VQA_format = "Please convert this plain text input into a VQA dataset format. The plain text response is - " + response + ". You need to create a dictionary containing two keys: 'prompt' and 'response'. \
                          The 'response' key should contain the given plain text input as it is. \
                              The 'prompt' key should be a well-crafted prompt that says what the image is of and asks the model to generate a detailed report of the image."
      ...
      print("Generating a VQA pair from the image and rough description")
      ...
      system_prompt_content_for_question_answer_pair_generation = "You are an AI assistant with advanced capabilities in radiology. \
                              Your task is to analyze the attached medical image and the associated rough description of it. \
                              Based on your analysis, you need to prepare a good quality visual question and its answer. \
                              The question should be relevant to the image and the answer should be a detailed explanation of the question. \
                              Both of them should be short, to the point, and informative. \
      ...
      user_prompt_content_question_answer_pair_generation = "Please process this image attached. Here is a rough description of it saying - |||" + item + "|||. I need you to analyze the image, combine knowledge from the description, and generate a good quality visual question and its answer. \
      ...
      if counter == limit_count:
        print("Finished processing the samples")
        break
    except Exception as e:
      print("Error processing the image: ", filename)
      print(e)
...
with open('roco_vqa.json', 'w') as f:
    json.dump(vqa_data, f, indent=2)

print("Saved the dataset in roco_vqa.json")